...
La page `/links/` regroupe les liens publics sous forme de grands boutons. Elle est accessible par « Links » dans le pied de page des autres pages, sans entrée supplémentaire dans le menu principal. La page Links elle-même n’affiche ni en-tête ni pied de page ; cette exception est gérée dans `PageShell`. Le bouton « Site officiel du club » revient à l’accueil avec le logo CAN ; le bouton « Contacter le club » ouvre l’adresse e-mail et le lien « Retour à l’accueil du site » revient aussi à l’accueil. Le libellé de ce dernier se modifie dans `backLabel` de `src/data/links.ts`. Modifier les libellés, descriptions et destinations dans `src/data/links.ts` ; le header, la page Musculation et le footer utilisent le compte commun, la page Force et le lien force athlétique utilisent `@can_powerlifting`, les liens Instagram lutte et WhatsApp restent documentés dans `docs/CLUB.md`.
