...
export function Discipline({ discipline }: { discipline: DisciplineData }) {
  ...
  const instagramUrl = isForce ? 'https://www.instagram.com/can_powerlifting/' : isWrestling ? 'https://www.instagram.com/can_lutte_nantes/' : site.contact.instagramUrl;
  return (
    <article className="discipline">
      <header className="discipline__hero">
        ...
        {(isForce || isWrestling || discipline.id === 'musculation') && <a className="discipline__instagram" href={instagramUrl} target="_blank" rel="noreferrer"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.5" r="1" /></svg>Nous suivre sur Instagram ↗</a>}
      </header>
